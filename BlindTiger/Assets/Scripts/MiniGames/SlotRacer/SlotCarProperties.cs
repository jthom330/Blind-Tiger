using UnityEngine;

public class SlotCarProperties : MonoBehaviour
{
    public const float DEFAULT_LAP_TICK_COUNTDOWN_TIME = 3.0f;
    private bool canIncrementLap;

    public int lapCount;
    public float lapCountdownTime;

    // Start is called before the first frame update
    private void Start()
    {
        lapCountdownTime = DEFAULT_LAP_TICK_COUNTDOWN_TIME;
    }

    // Update is called once per frame
    private void Update()
    {
        lapCountdownTime -= Time.deltaTime;

        if (lapCountdownTime <= 0) canIncrementLap = true;
    }

    private void OnCollisionEnter(Collision aCollisionObject)
    {
        if (aCollisionObject.collider.tag == "finishline")
            if (canIncrementLap)
                IncrementLap();
    }

    private void IncrementLap()
    {
        lapCount++;
        ResetLapCountdown();
        Debug.Log(lapCount);
    }

    private void ResetLapCountdown()
    {
        canIncrementLap = false;
        lapCountdownTime = DEFAULT_LAP_TICK_COUNTDOWN_TIME;
    }
}